...
function main() {
    ...
    $.getJSON("/themes/theme.json", function(data){
    ...
    $.getJSON("/img/wallpaper.json", function(data) {
                    ...
                    success: (function(data) {
                        ...
                        switch (menu) {
                            ...
                            case 3:
                                ...
                                preImg(["/img/marioPoster.jpg",
                                        "/img/zeldaPoster.jpg",
                                        "/img/dkPoster.jpg",
                                        "/img/metroidPoster.jpg"
                                        ]);
                                getVideo(0);
                                videoList();
                                break;
...
function videoList() {
    ...
    $.getJSON("/videos/video.json", function(data) {
...
//Cria lista de videos
function musicList() {
    $.getJSON("/songs/song.json", function(data) {
...
function gameList() {
    ...
    $.getJSON("/games/game.json", function(data) {
...
function getGame(n){
    $.getJSON("/games/game.json", function(data) {
            $("#mudar").html("<iframe id='igame' src='/games/gba/launcher.html#" + data.g[n].src+"' ></iframe>");
        })
        .done(function() {
            //Em caso de sucesso
...
function init(){
    $.getJSON("/users/info.json",function(data) {
        data.forEach(function(e,i){
            $("#areaUser").append("<figure onclick='ativaUser(this)' id='"+i+"'><img src='/users/img/"+e.avatar+"'><figcaption>"+e.nome+"</figcaption></figure>");
        });
    });
}
...
function getUser(num){
    $.getJSON("/users/info.json",function(data){
        ...
        $("#user img").attr("src", "/users/img/"+data[num].avatar);
        $("#user figcaption").html(data[num].nome);
        ...
        if(data[num].tipo == 1){
            $("#bgFoto").css("background-image", "url(/img/"+data[num].wall+")").fadeIn("slow");
            $(".bg").fadeOut();
        }else{
